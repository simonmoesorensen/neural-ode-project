...
class LSTMBaseline(TrainerModel):
    # https://towardsdatascience.com/step-by-step-understanding-lstm-autoencoder-layers-ffab055b6352
    def __init__(self, input_dim, hidden_dim, layer_dim, device=None):
        super(TrainerModel, self).__init__()
        ...
        # Model
        self.lstm = nn.LSTM(input_dim, hidden_dim, layer_dim, batch_first=True).to(self.device)
    ...
    def init_hidden(self, batch):
        h = torch.zeros(self.layer_dim, batch, self.hidden_dim)
        c = torch.zeros(self.layer_dim, batch, self.hidden_dim)
        return [h, c]
    
    def train_step(self, x, t):
        pred_x = self.forward(x, self.device)
        loss = torch.sqrt(self.criterion(x, pred_x))
        return loss

    @classmethod
    def from_checkpoint(cls, path):
        # Usage: LSTMBaseline.from_checkpoint(path)
        obj = torch.load(path)
        model = cls(**obj['model_args'])
        model.lstm.load_state_dict(obj['lstm_state_dict'])
        model.fc.load_state_dict(obj['fc'])
...
class ODEAutoEncoder(TrainerModel):
    def __init__(self, latent_dim, obs_dim, hidden_dim, rnn_hidden_dim=None, lstm_hidden_dim=None,  device=None, solver='rk4'):
        super(TrainerModel, self).__init__()
